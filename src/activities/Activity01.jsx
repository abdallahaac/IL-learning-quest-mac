...
import { downloadNotesAsWord } from "../components/NoteComposer.jsx"; // exporter
...
export default function Activity01({
	...
	// ---- DOWNLOAD state to prevent repeated downloads ----
	const [isDownloading, setIsDownloading] = useState(false);

	// localized strings used for the generated document
	const docLocaleStrings = {
		en: {
			reflectionLabel: "Reflection",
			resourcesHeading: exportLinksHeading || "Resources",
			defaultIntro:
				"Notes and reflections from the activity. Use this document to keep or share your learning.",
			dateLabel: "Date",
		...
		fr: {
			...
			resourcesHeading: exportLinksHeading || "Ressources",
			defaultIntro:
				"Notes et réflexions de l’activité. Utilisez ce document pour conserver ou partager vos apprentissages.",
			dateLabel: "Date",
	...
	const handleDownload = async () => {
		// prevent downloads when there's no content or a download is already running
		...
		try {
			await Promise.resolve(
				downloadNotesAsWord({
					...
					docIntro,
					includeLinks,
					linksHeading: docLocaleStrings[lang].resourcesHeading,
					pageLinks: linkItems,
					headingColor: accent,
					accent,
					// localization hints for the exporter (ignored safely if unsupported)
					locale: lang,
					localeStrings: docLocaleStrings[lang],
				})
			);
		} catch (err) {
	...
	const fadeUp = pageFade; // small alias
		...
		>
			...
			<div className="max-w-5xl mx-auto px-4 py-8 sm:py-12 space-y-6">
				...
				<NoteComposer
					...
					onRequestDownload={handleDownload} // ensures any internal download uses same handler
				/>
				...
				<div className="flex justify-end gap-2">
					...
					<DownloadButton
						...
						accent={accent}
						label={L.downloadDoc}
						downloadingLabel={docLocaleStrings[lang].downloading}
